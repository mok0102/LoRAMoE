...
class ModelArguments:
    ...
    model_base: Optional[str] = field(
        ...
        metadata={
            "help": (
                "The model checkpoint for weights initialization base."
            )
        },
    )
...
def main():
    ...
    if model_args.config_name:
        ...
        config = AutoConfig.from_pretrained(model_args.model_base, **config_kwargs)
    else:
    ...
    if training_args.do_train:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            files = [os.path.join(path,file.name) for file in path.glob("*.json")] # train.json
    ...
    if training_args.peft_path is not None: # --------------------------> train from the trained lora model
        ...
        peft_config = LoraConfig(
            ...
            inference_mode=True if eval_dataset is not None else False,
            r=lora_rank, 
    ...
    if training_args.do_eval:
        ...
        # predoutput = trainer.predict(eval_dataset) ### model initialize 
        # 빼오기!
    
        trainer.make_eval_mode(eval_dataset)
        
        savedict=[]
        for i in range(len(eval_dataset)):
            input_ = eval_dataset[i]['input_ids'].unsqueeze(0)
            output_ = eval_dataset[i]['labels'].unsqueeze(0)
            # print('inoutput', input_.shape, output_.shape)
            n_diff_input_output = (input_.squeeze() != output_.squeeze()).sum().item()
            # n_diff_input_output = output_.shape[1]- input_.shape[1]
            print(n_diff_input_output, 'diff')
            
            
            test = model.generate(
                inputs=input_[:, :n_diff_input_output].cuda(),
                # max_length=1024,
                # max_new_tokens=70,
                use_cache=False
            )
            # print('ingen', input_.shape, test.shape)
        
            output = tokenizer.batch_decode(test[:, n_diff_input_output:], skip_special_tokens=True)

            # print('pred_using generate', output)
            
            savedict.append({
                'input': tokenizer.decode(input_[:, :n_diff_input_output].squeeze().tolist(), skip_special_tokens=True),
                'output': output[0],
                'target': tokenizer.decode(output_[:, n_diff_input_output:].squeeze().tolist(), skip_special_tokens=True),
            })
            
            print(savedict[-1]['output'])
            print(savedict[-1]['target'])
            print()            
            with open(os.path.join(training_args.output_dir, f"pred.json"), 'w') as f:
